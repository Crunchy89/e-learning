...
class Menu extends MY_Controller
{
	...
	function getLists()
	{
		...
		foreach ($menu as $d) {
			...
			if ($d->is_active == 1) {
				$active = '<input type="checkbox" name="active" ' . $disabled . '  class="form-control " data-id_menu="' . $d->id_menu . '" data-active="' . $d->is_active . '" form-control-sm" id="active" checked>';
			} else {
				$active = '<input type="checkbox" name="active" ' . $disabled . ' class="form-control " data-id_menu="' . $d->id_menu . '" data-active="' . $d->is_active . '" form-control-sm" id="active" >';
			}
